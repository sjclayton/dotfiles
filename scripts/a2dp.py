...
Fixing bluetooth stereo headphone/headset problem in ubuntu 16.04 and 
also debian jessie, with bluez5.

Workaround for bug: 
https://bugs.launchpad.net/ubuntu/+source/indicator-sound/+bug/1577197
Run it with python3.5 or higher after pairing/connecting the bluetooth 
stereo headphone.
 ...
 * https://github.com/DominicWatson, for adding the ``-p/--profile`` 
argument.
 * https://github.com/IzzySoft, for mentioning wait before connecting 
again.
...
- 0.4.0
  ...
  * Sending all available streams into selected sink, after successfull 
connection by @AmploDev.
...
- 0.3.0
  * Adding -p / --profile option for using the same script to switch 
between headset and A2DP audio profiles
...
- 0.1.1
  * Supporting the `[NEW]` prefix for devices & controllers as advised 
by @wdullaer
  * Drying the code.
...
DEVICE_PATTERN = 
re.compile('^(?:.*\s)?Device\s(?P<mac>%s)\s(?P<name>.*)' % 
MAC_ADDRESS_PATTERN)
CONTROLLER_PATTERN = 
re.compile('^(?:.*\s)?Controller\s(?P<mac>%s)\s(?P<name>.*)' % 
MAC_ADDRESS_PATTERN)
WAIT_TIME = 2.25
TRIES = 15
PROFILE = 'a2dp'
...
parser.add_argument('-e', '--echo', action='store_true', default=False,
                    help='If given, the subprocess stdout will be also 
printed on stdout.')
parser.add_argument('-w', '--wait', default=WAIT_TIME, type=float,
                    help='The seconds to wait for subprocess output, 
default is: %s' % WAIT_TIME)
parser.add_argument('-t', '--tries', default=TRIES, type=int,
                    help='The number of tries if subprocess is failed. 
default is: %s' % TRIES)
parser.add_argument('-p', '--profile', default=PROFILE,
                    help='The profile to switch to. available options 
are: hsp, a2dp. default is: %s' % PROFILE)
...
class BluetoothctlProtocol(asyncio.SubprocessProtocol):
    ...
    async def search_in_output(self, expression, fail_expression=None):
        ...
        for l in self.output.splitlines():
            if fail_expression and re.search(fail_expression, l, 
re.IGNORECASE):
                raise SubprocessError('Expression "%s" failed with fail 
pattern: "%s"' % (l, fail_expression))

            if re.search(expression, l, re.IGNORECASE):
                return True

    async def send_and_wait(self, cmd, wait_expression, 
fail_expression='fail'):
        try:
            self.listen_output()
            await self.send_command(cmd)
            while not await 
self.search_in_output(wait_expression.lower(), 
fail_expression=fail_expression):
    ...
    async def disconnect(self, mac):
        ...
        await self.send_and_wait('disconnect %s' % ':'.join(mac), 
'Successful disconnected')

    async def connect(self, mac):
        print('Connecting again.')
        await self.send_and_wait('connect %s' % ':'.join(mac), 
'Connection successful')

    async def trust(self, mac):
        await self.send_and_wait('trust %s' % ':'.join(mac), 'trust 
succeeded')
    ...
    async def select_paired_device(self):
        ...
        selected = input()
        return devices[0 if not selected.strip() else (int(selected) - 
1)]
...
async def execute_command(cmd, ignore_fail=False):
    p = await asyncio.create_subprocess_shell(cmd, stdout=sb.PIPE, 
stderr=sb.PIPE)
...
async def find_dev_id(mac, **kw):
    return await execute_find('pactl list cards short', 'bluez_card.%s' 
% '_'.join(mac), **kw)


async def find_sink(mac, **kw):
    return await execute_find('pacmd list-sinks', 'bluez_sink.%s' % 
'_'.join(mac), **kw)
...
async def set_profile(device_id, profile):
    ...
    try:
        return await execute_command('pactl set-card-profile %s %s' % 
(device_id, _profiles[profile]))
    except KeyError:
        print('Invalid profile: %s, please select one one of a2dp or 
hsp.' % profile, file=sys.stderr)
        raise SystemExit(1)
...
async def move_streams_to_sink(sink):
    streams = await execute_command('pacmd list-sink-inputs | grep 
"index:"', True)
    for i in streams.split():
        i = ''.join(n for n in i if n.isdigit())
        if i != '':
            print('Moving stream %s to sink' % i)
            await execute_command('pacmd move-sink-input %s %s' % (i, 
sink))
    return sink
...
asyncio.get_event_loop().subprocess_exec(
        lambda: BluetoothctlProtocol(exit_future, echo=args.echo), 
'bluetoothctl'
    )
    ...
    try:
        ...
        if device_id is None:
            print('It seems device: %s is not connected yet, trying to 
connect.' % ':'.join(mac))
